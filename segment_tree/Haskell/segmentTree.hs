class (Show a) => STNodeValue a where 
    neutralValue :: a
...
leaf :: (STNodeValue a) => Int -> a -> STNode a
...
mergeNodes :: (STNodeValue a) => STNode a -> STNode a -> STNode a
mergeNodes ln rn = Node { value = mergeValues (value ln) (value rn),
                            lnode = ln,
                            rnode = rn,
...
build :: (STNodeValue a) => Int -> STNode a  
build n = build' 0 (realN - 1)  where 
    realN = go n 1 where go n p = if p >= n then p else go n (p*2) 

    build' :: (STNodeValue a) => Int -> Int -> STNode a
    build' l r 
        | l == r    = leaf l neutralValue
...
updateElement :: (STNodeValue a) => STNode a -> Int -> a -> STNode a
updateElement t i v 
    | lb t == rb t = t {value = v} 
...
get :: (STNodeValue a) => STNode a -> Int -> Int -> a
get t l r 
    | lb t == l && rb t == r = value t
    | l > r        = neutralValue 
    | otherwise    = mergeValues lvalue rvalue where 
        m = (lb t + rb t) `div` 2 
